 ...
 * or photo. They call no_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Norrböle Sten & Kross AB.
 *
 ...
 * @return string
 */
function no_sample_note() {
	return no_lead( 'ar_riktig' ) ? '' : ' <span class="no-sample">(platshållare)</span>';
}
 ...
 * @return string File name for no_lead_img().
 */
function no_lead_bild( $i ) {
	$bilder = (array) no_lead( 'bilder', array() );
 ...
 * @return string
 */
function no_ring_label() {
	$namn = no_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, no_lead( 'telefon' ) ) : 'Ring ' . no_lead( 'telefon' );
}
