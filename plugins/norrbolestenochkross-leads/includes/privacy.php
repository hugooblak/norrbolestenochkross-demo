...
add_filter(
	...
	function ( $exporters ) {
		$exporters['norrbolestenochkross-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'norrbolestenochkross-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'norrbolestenochkross-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'norrbolestenochkross-leads',
						'group_label' => __( 'Quote requests', 'norrbolestenochkross-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['norrbolestenochkross-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'norrbolestenochkross-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
