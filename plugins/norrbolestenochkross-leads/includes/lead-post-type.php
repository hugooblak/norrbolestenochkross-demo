...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'norrbolestenochkross-leads' ),
					'singular_name' => __( 'Lead', 'norrbolestenochkross-leads' ),
					'edit_item'     => __( 'Förfrågan', 'norrbolestenochkross-leads' ),
					'search_items'  => __( 'Search leads', 'norrbolestenochkross-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'norrbolestenochkross-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'norrbolestenochkross-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'norrbolestenochkross-leads' ),
			'npl_phone'    => __( 'Phone', 'norrbolestenochkross-leads' ),
			'npl_service'  => __( 'Needs', 'norrbolestenochkross-leads' ),
			'npl_timeline' => __( 'When', 'norrbolestenochkross-leads' ),
			'npl_zip'      => __( 'ZIP', 'norrbolestenochkross-leads' ),
			'npl_source'   => __( 'Source', 'norrbolestenochkross-leads' ),
			'npl_status'   => __( 'Status', 'norrbolestenochkross-leads' ),
			'date'         => __( 'Received', 'norrbolestenochkross-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'norrbolestenochkross-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'norrbolestenochkross-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'norrbolestenochkross-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'norrbolestenochkross-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'norrbolestenochkross-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'norrbolestenochkross-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'norrbolestenochkross-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'norrbolestenochkross-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'norrbolestenochkross-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'norrbolestenochkross-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'norrbolestenochkross-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'norrbolestenochkross-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
